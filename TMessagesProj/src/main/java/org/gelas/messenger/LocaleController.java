...
import org.gelas.Pouya.utils.TimeAgo;
import org.gelas.Pouya.utils.shamsicalendars.ShamsiCalendar;
import org.gelas.Pouya.utils.shamsicalendars.ShamsiDate;
import org.gelas.messenger.time.FastDateFormat;
...
public class LocaleController {
    ...
    public static String formatYear(long j) {
        ...
        return ( Setting.getDatePersian()) ? ShamsiCalendar.dateToShamsi(new Date(1000 * j)).toDateString() : getInstance().formatterYear.format(new Date(1000 * j));
    }
    ...
    public static String formatDateChat(long date) {
        if(Setting.getShowTimeAgo()){
            return  TimeAgo.timeAgo(date*1000);
        }
        try {
    ...
    public static String formatDateOnline(long date) {
        ...
        if (Setting.getDatePersian()) {
            try {
                ...
                ShamsiDate dateToShamsi = ShamsiCalendar.dateToShamsi(new Date(date * 1000));
                if (i4 == i && i2 == i3) {
                    return String.format("%s %s %s", new Object[]{getString("LastSeen", R.string.LastSeen), getString("TodayAt", R.string.TodayAt), getInstance().formatterDay.format(new Date(date * 1000))});
                } else if (i4 + QUANTITY_ZERO == i && i2 == i3) {
                    return String.format("%s %s %s", new Object[]{getString("LastSeen", R.string.LastSeen), getString("YesterdayAt", R.string.YesterdayAt), getInstance().formatterDay.format(new Date(date * 1000))});
                } else if (i2 == i3) {
                    Object[] objArr2 = new Object[QUANTITY_ONE];
                    objArr2[QUANTITY_OTHER] = dateToShamsi.getDay() + " " + (language.toLowerCase().equals("fa") ? ShamsiCalendar.getShamsiMonth(dateToShamsi.getMonth()) : ShamsiCalendar.getShamsiMonthEn(dateToShamsi.getMonth()));
                    objArr2[QUANTITY_ZERO] = getInstance().formatterDay.format(new Date(date * 1000));
                    ...
                    objArr[QUANTITY_ZERO] = getInstance().formatterDay.format(new Date(date * 1000));
